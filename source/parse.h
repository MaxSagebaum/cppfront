...
std::vector<std::string> cpp1_libraries = {};

using meta_create_func = std::function<bool(std::unique_ptr<declaration_node>&)>;
...
public:
    ...
    //  errors      error list
    //
    parser( std::vector<error_entry>& errors_, meta_create_func const& meta_create_ )
        : errors{ errors_ }
        , parse_tree{std::make_unique<translation_unit_node>()}
        , meta_create {meta_create_}
    { }

    parser( parser const& that )
        : errors{ that.errors }
        , parse_tree{std::make_unique<translation_unit_node>()}
        , meta_create {that.meta_create}
    { }
...
private:
        ...
        -> std::unique_ptr<translation_unit_node>
    {
        ...
        for (auto d = declaration(); d; d = declaration()) {
            if (d->is_metafunction()) {
                if(!meta_create(d)) {

                    // Metafunction compilation is not active add the metafunction.
